...
// 图例echarts
let myChartTwo  = null
const setTooltipEchart = (params:any,data:any) => {
  console.log("图例echarts",params);
  ...
  const option = {
    ...
    xAxis: {
      type: 'category',
      boundaryGap: false,
      data: currentData.dateName.slice(-7),
      splitLine: {
        show: false
      },
      axisTick: {
    ...
    series: [
      ...
      },
      {
        ...
        data: currentData.zjs.slice(-7)
      },
    ]
  }
  nextTick(()=>{//然后setoption方法生成，记住这里一定要用$nextTick不然你拿不到dom节点
    myChartTwo = echarts.init(document.getElementById("tooltipChart"));
    ...
    var option: ECOption = {
      ...
      tooltip: {
        ...
        formatter: (params: Record<string, any>) => {
          var htmlStr = `<div id="tooltipChart" ref='tooltipEchart' style='width:420px;height:240px;'></div>`;
          // setTooltipEchart(params);
          // console.log("ddddddd",params)
          http.get(`/renshedata/apiController/yxjs?qy=${params.data.name}`).then(({ code, data }) => {
            if (code === 200) {
              console.log("图例echarts",data)
              setTooltipEchart(params,data);
            }
          })
          return `
          <div 
          ...
          <div  
            ...
            color: #FFFFFF;
          ">${params.name}</div>
          <div style="font-size:14px;margin-top:10px">总件数：${params.data.zjs} <span style="margin-left:10px">有效件数：${params.data.yxj}</span></div>
          <div style="font-size:14px;margin-top:10px">重复件：${params.data.cfj} <span style="margin-left:10px">区占比：${params.data.zb}</span></div>
          </div>
